...
PortM_Init 
		;STEP 1 Activate clock (Set bit 5 in RCGCGPIO; In C pseudcode: SYSCTL_RCGCGPIO_R |= #0x20)
		 LDR R1, =SYSCTL_RCGCGPIO_R		;Stores the address of the RCGCGPIO register in R1
		 LDR R0, [R1]					;Dereferences R1 to put the contents of the RCGCGPIO register in R0
		 ORR R0,R0, #0x0800				;Modifies the contents of R0 to set bit 10 without changing other bits
		 STR R0, [R1]					;Stores the new value into the RCGCGPIO register
		...
		LDR R1, =GPIO_PORTM_DIR_R		;Load the memory address of the GPIODIR Port L Register into R1 (pointer)
		...
		LDR R1, =GPIO_PORTM_DEN_R		;Load the memory address of the GPIODEN Port L Register into R1 (pointer)
...
Start 
		...
		BL PortM_Init		; call and execute PortL_Init
		
		;STEP 5: Outputting a Logic High to Port F Pin 4 (to switch on D3)   

		
WaitForButtonPress 
		LDR R1, =GPIO_PORTM_DATA_R	;Load the memory address of the GPIODATA Port L Register into R1 (pointer)
...
LightON
		...
		LDR R0, [R1]
		ORR R0, R0, #0x10				;Clear bit 4 on Port F to switch off D3
		STR R0, [R1]
	
WaitForButtonRelease 
		LDR R1, =GPIO_PORTM_DATA_R		;Go through a similar sequence to what we did to check if button was pushed
		LDR R0, [R1]
		AND R0, R0, #0x01
		CMP R0, #1								;The only difference is we're comparing with 1 (button released) instead of 0
		...
		B WaitForButtonPress
											;We only get here when button is released, so go back to beginning						;
